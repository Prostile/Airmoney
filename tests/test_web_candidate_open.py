...
def test_candidate_details_page_shows_raw_text_and_analysis(tmp_path, monkeypatch):
    monkeypatch.setenv("AIRMONEY_WEB_USER", "admin")
    monkeypatch.setenv("AIRMONEY_WEB_PASSWORD", "secret")
    repo = Repository(tmp_path / "test.sqlite3")
    repo.save_collection(Collection(id="c1", name="Collection"))
    ...
    candidate.exact_item_match = True
    repo.save_candidate(candidate)
    app = create_app(repo)

    with TestClient(app) as client:
        response = client.get(f"/candidates/{candidate.id}", headers=_auth_header())

    assert response.status_code == 200
    assert "Skin A raw card text" in response.text
    assert "Mil-Spec" in response.text
    assert "Exact item match" in response.text
